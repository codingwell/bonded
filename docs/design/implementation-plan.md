...
**Status:** In Progress
**Last Updated:** 2026-05-30 (session 61)
...
| 7.4 | Implement mDNS advertiser and browser | in-progress | `PeerShareAdvertisement` now includes server affinity, listener endpoint, listener cert fingerprint, and instance nonce in TXT records. `bonded-client::peer_runtime` starts an mDNS advertiser/browser, filters discovered peers by paired server and self device key, requests signed introductions for matching peers, and dials accepted peers into `PeerRelay` transports. Runtime teardown/reconnect behavior is still pending. |
| 7.5 | Wire peer paths into scheduler/runtime | in-progress | `ClientRuntime` now starts a background peer-share runtime on Linux when `client.peer_share_enabled = true`, and the TUN loop accepts dynamically injected `PeerRelay` transports through a channel. Session 61 adds a provider->server relay-registration control frame carrying the server-signed introduction, and the server now verifies that control frame before binding the dedicated upstream provider transport to the introduced consumer session. This removes the earlier consumer-identity collapse at the server boundary. Remaining work in 7.5 is runtime teardown/reconnect hardening and choosing whether providers should eventually relay over existing live transport pools instead of one dedicated upstream path per consumer. |
| 7.6 | Validate bidirectional two-device peer sharing | not-started | Use the two ADB-connected devices to prove discovery, signed introduction, relay establishment, and bidirectional shared-path failover. |
