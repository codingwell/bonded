...
**Status:** In Progress
**Last Updated:** 2026-05-13 (session 53)
...
| Real-device local WireGuard validation against the devcontainer host is blocked by incomplete server-observed session establishment | **open** | Session 55 resolved the original LAN reachability problem: the phone can now open raw TCP connections to `192.168.1.185:443` and `:8000`, pairing succeeds directly against the devcontainer host, and the device reaches `transport[0] = WireGuard` with `state=connected`. During that run two bootstrap bugs were fixed server-side: REST handlers now send TLS `close_notify` before closing, and the bootstrap listener now reads HTTP request bodies before routing POST JSON. The remaining gap is that the server status endpoint still reports `sessions_count=0` and the server logs show WireGuard peer registration but no authenticated session/traffic after transport establishment, so end-to-end WireGuard validation is not yet complete. |
