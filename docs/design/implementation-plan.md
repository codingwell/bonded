...
**Status:** In Progress
**Last Updated:** 2026-05-01 (session 49)
...
| Future perf target: reduce smoltcp poll retry churn under zero-progress sends | 2026-04-30 | `SmoltcpForwarder` currently retries on a short timed poll cadence (~1ms wait path in `smoltcp_poll_thread`) when `send_slice` returns zero. This is functionally correct but a potential CPU wakeup hotspot under sustained backpressure; consider event-driven wakeups or adaptive backoff while pending bridge bytes remain unsent |
