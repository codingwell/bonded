...
| 6.3.4 | Implement WG transport adapter | completed | `WireGuardTransport` implements `Transport` trait in `bonded-core`; `establish_wireguard_session()` in `bonded-client`; `ClientTransport::WireGuard` variant; `("wireguard" | "wg")` arm in `establish_transport_paths` |
