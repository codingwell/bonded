...
| 4.11 | End-to-end test: Android → server → internet | completed | Host-side FFI smoke tests pass (50/50 including bind-address assertion); `libbonded_ffi.so` built for arm64-v8a and x86_64 and placed in jniLibs; debug APK builds successfully; device validation is now complete on a physical Android device using the release APK and `DebugVpnActivity`: Kotlin pre-resolves `charter.codingwell.net` to `97.115.185.254:8080`, Rust dials the resolved address while preserving the hostname for TLS/cert-proof verification, logcat shows `Transport paths established: count=1` with `transport[0] = WebSocketTLS`, and the session snapshot reaches `state=connected` with non-zero inbound/outbound traffic counters |
